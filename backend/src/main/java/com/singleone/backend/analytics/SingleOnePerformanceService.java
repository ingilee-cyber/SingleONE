...
/**
 * PRD 8장 SingleONE 성과/Index 계산 오케스트레이션. 프로젝트의 캠페인 구성(MySQL)과 원본 성과
 * (ClickHouse)를 조회해 {@link SingleOneIndexCalculator}에 위임한다.
 */
@Service
public class SingleOnePerformanceService {
	...
	public SingleOnePerformanceService(ProjectCampaignRepository projectCampaignRepository,
	...
	public List<MediaIndexResult> calculatePeriod(Long projectId, String advertiserId, LocalDate from, LocalDate to) {
		List<ProjectCampaignId> campaigns = projectCampaigns(projectId);
		Set<Media> projectMedia = projectMedia(campaigns);

		List<DailyMediaTotal> daily = aggregationRepository.fetchDailyMediaTotals(advertiserId, campaigns, from, to);
	...
	public PeriodComparison calculatePeriodWithPreviousComparison(Long projectId, String advertiserId,
			LocalDate from, LocalDate to) {
		List<MediaIndexResult> current = calculatePeriod(projectId, advertiserId, from, to);
		long lengthDays = ChronoUnit.DAYS.between(from, to) + 1;
		LocalDate previousTo = from.minusDays(1);
		LocalDate previousFrom = previousTo.minusDays(lengthDays - 1);
		List<MediaIndexResult> previous = calculatePeriod(projectId, advertiserId, previousFrom, previousTo);
	 ...
	 */
	public List<RollingIndexPoint> calculateRollingIndex(Long projectId, String advertiserId, LocalDate from, LocalDate to) {
		List<ProjectCampaignId> campaigns = projectCampaigns(projectId);
		Set<Media> projectMedia = projectMedia(campaigns);

		LocalDate fetchFrom = from.minusDays(6);
		List<DailyMediaTotal> daily = aggregationRepository.fetchDailyMediaTotals(advertiserId, campaigns, fetchFrom, to);
	...
	private List<ProjectCampaignId> projectCampaigns(Long projectId) {
		return projectCampaignRepository.findByIdProjectId(projectId).stream().map(ProjectCampaign::getId).toList();
	}
